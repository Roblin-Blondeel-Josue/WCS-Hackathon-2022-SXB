...
function CarbonPrintCalcul(distance, voyageur) {
  const carbonPrintResult = Math.round(distance * 0.285 * voyageur);
...
export default function CarbonPrint({ depart, airport, name }) {
  const [voyageur, setVoyageur] = React.useState(1);
  const handleChange = (e) => {
    setVoyageur(e.target.value);
  };
  const distanceParcourue = MakeVoyage(depart, airport);
  const carbonPrintFlight = CarbonPrintCalcul(distanceParcourue, voyageur);
  ...
  return (
    <Card sx={styleCardMd}>
      ...
      >
        ...
        />
        Empreinte carbone de votre vol
      </Typography>
      <Typography
        variant="body2"
        align="center"
      ...
      <FormControl
        ...
        variant="outlined"
      >
        <InputLabel id="filter-meals">Voyageur :</InputLabel>
        <Select
      ...
      >
        <CloudIcon
          ...
          sx={matches ? { fontSize: "2em" } : { fontSize: "1em" }}
        />
        L'équivalant de {carbonPrintCow} vaches pétant une journée.
      </Typography>
    </Card>
  );
}
